...
class IPCamera(object):
	...
	def __init__(self,camInfo, cameraFunction, dlibDetection, fpsTweak):
		...
		nn_budget = None
		self.tracker = Tracker(nn_matching.NearestNeighborDistanceMetric("cosine", max_cosine_distance, nn_budget))
		...
		if self.camURL == "0" or self.camURL == "1":
    			self.video = cv2.VideoCapture(int(self.camURL))
		else:	
				self.video = cv2.VideoCapture(self.camURL) # VideoCapture object used to capture frames from IP camera
		logger.info("We are opening the video feed.")
		self.url = self.camURL
	...
	def get_frame(self):
		...
		while True :
			...
			if success:		
				...
				print("Cant grab frame of camera ",self.camURL)
				self.video.release()
				break
	...
	def show_processed_image(self):
		cv2.imshow("Preview", self.processing_frame)
