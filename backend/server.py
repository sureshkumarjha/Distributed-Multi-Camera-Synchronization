...
CORS(app,resources={r"/connect": {"origins": "http://localhost:"}})
...
def user_login():
    ...
    if request.method == 'POST':
        ...
        if request.json['userName'] != 'admin' or request.json['password'] != 'admin@123':
            ...
            camera_list = []
            for cam in HomeSurveillance.cameras:
                camera_list.append(cam.camInfo)
            
            return { 
                "loginStatus" : True, 
            "total_camera": len(HomeSurveillance.cameras),
            "camera_list" : camera_list
            }

@app.route('/react_add_camera', methods = ['GET','POST'])
def react_add_camera():
    ...
    if request.method == 'POST':
        try:  
            ...
            with HomeSurveillance.camerasLock :
                HomeSurveillance.add_camera(SurveillanceSystem.Camera.IPCamera(camInfo,application,detectionMethod,fpsTweak))
            data = {"camNum": len(HomeSurveillance.cameras) -1, "status": True }
            return data
...
@app.route('/react_remove_camera', methods = ['GET','POST'])
def react_remove_camera():
    try:
        if request.method == 'POST':
            camID = request.json.get('camID')
            print(camID)
...
def retrain_classifier():
    if request.method == 'POST':
        ...
        for i, camera in enumerate(HomeSurveillance.cameras):
            print(camera.captureThread.stop)
        # Release processing threads       
        data = {"finished":  "","camNum": 0}
